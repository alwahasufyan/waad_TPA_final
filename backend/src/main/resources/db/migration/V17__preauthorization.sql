-- ============================================================
-- V17: الموافقات المسبقة (Pre-Authorization)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_pauthreq_member_status_date
    ON preauthorization_requests(member_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_pauthreq_expiring
    ON preauthorization_requests(valid_until)
    WHERE status = 'APPROVED' AND valid_until IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_pauthreq_provider_date
    ON preauthorization_requests(provider_id, created_at DESC, status);

-- الموافقات المسبقة — بعد V229: محذوف medical_service_id — بعد V223: مضاف email_request_id
