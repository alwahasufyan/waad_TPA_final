    ...
    for _, row in df_in.iterrows():
        ...
        if name_alt and name_alt.lower() != name.lower():
            ...
            if res_alt["score"] > res["score"]:
                res = res_alt
        if (str(res.get("sub") or "").startswith("CAT-LAB") and
                str(res.get("method") or "").lower().startswith("fuzzy")):
            res = dict(res)
            res["score"] = min(float(res.get("score") or 0),
                                max(0.0, float(threshold) - 0.01))
        ...
        if res["score"] >= threshold:
            ...
            matched_rows.append({
                ...
                COL_SUB: asub,
                COL_NOTE: anote,
                COL_STATUS: STATUS_OK if category_source in ("ref", "odoo", "knowledge") else STATUS_REVIEW,
        ...
        if csrc in ("hint", "odoo", "knowledge"):
